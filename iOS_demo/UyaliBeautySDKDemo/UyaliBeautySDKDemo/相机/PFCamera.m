...
{
    if (!_camera) {
        _camera = [self cameraWithPosition:self.cameraPosition];
        if (_camera) {
            [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(subjectAreaDidChange:) name:AVCaptureDeviceSubjectAreaDidChangeNotification object:_camera];
        }
    }
...
- (AVCaptureVideoDataOutput *)videoOutput
{
    if (!_videoOutput) {
        ...
        [_videoOutput setVideoSettings:[self videoSettingsForCaptureFormat]];
        [_videoOutput setSampleBufferDelegate:self queue:self.videoCaptureQueue];
    }
    return _videoOutput;
}

- (NSDictionary *)videoSettingsForCaptureFormat
{
    return @{
        (id)kCVPixelBufferPixelFormatTypeKey: [NSNumber numberWithInt:_captureFormat],
...
- (dispatch_queue_t)videoCaptureQueue {
    if (_videoCaptureQueue == nil) {
        _videoCaptureQueue = dispatch_queue_create("com.mmface.videoCaptureQueue", DISPATCH_QUEUE_SERIAL);
